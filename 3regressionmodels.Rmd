...
dat$X <- NULL
dat$ID <- NULL
# keep original data set stored because later, columns will be added to dat
orig_dat <- dat
```
## 1 - THE DATA
```{r}
...
We can see that there's a relationship between this variables but it seems like the progression is not quite linear but maybe quadratic or exponential.
...
plot(log(dat$price)~dat$sqft_above, xlab="sqft of the house", 
     ylab="log(price)")
```
We can see that there's a relationship between those variables although it doesn't seem quite linear.
...
model_1
```
We can see that sqft_living doesn't give us a result on the correlation, so we're going to remove it, which is something that already made sense to do since it had a very high correlation with sqft_above, which meant that they both gave very similar information.

```{r}
dat_red$sqft_living<-NULL
...
summary(model_1)
```
As we can see given our Pr(>|t|) results, all the variables are likely to have a relationship with price except for sqft_living15. This model gave us a 0.64 R-squared value, let's try to improve it.
## Model 2
Let's now try using dat_red_out, which is the dataset without removing the outliers.
```{r}
dat_red_out$sqft_living <- NULL
```
...
summary(model_2)
```
The dataset without the outliers (dat_red) gave us a significantly better result so we're gonna stick with it for the next iterations.

## Model 3
The effect of sqft_above (sqft of the house without the basement) may not be constant throughout all values (in fact we could see when plotting it with price that their relation wasn't quite linear), therefore we will introduce a higher-order term by creating a new variable.
```{r}
...
summary(model_3)
```
This model has a slightly better R-squared result than model 1. From 0.6414 to 0.6709.

## Model 4
It makes sense for houses with basement to have a higher price than houses without it. Let's add that new variable.
...
summary(model_4)
```
The R-squared value has barely improved.

## Model 5
It could be that grade has a combined impact with other variables, for example if the house has two or more floors and the grade is good, the price would be higher. 
```{r}
dat_red$floors2 <- ifelse(dat$floors >= 2, 1, 0)
dat_red$grade10 <- ifelse(dat_red$grade >= 10, 1, 0)

```
We've also added back waterfront with the premise that if the grade of the house is good and it has a view to a waterfront, the price will be higher.
```{r}
model_5 = lm(price ~ . +floors2*grade10+dat$waterfront*grade10, data=dat_red)
summary(model_5)
```

The model has improved the value of R-squared from 0.6712 to 0.6935.

### Model 6
Let's start again and use the dataset with all the variables, since even if they don't have a correlation with price it could be that some relationships between them do have a correlation.
```{r}
model_6 = lm(price ~ . , data=dat)
summary(model_6)
```
The R-squared value has significantly improved.

### Model 7
Let's now add the improvements that we used before.
```{r}
dat$sqft_basement<-NULL
```


```{r}
model_6 = lm(price ~ . +dat_red$sqft_above2 +dat_red$floors2*dat_red$grade10+waterfront*dat_red$grade10 , data=dat)
summary(model_6)
```
