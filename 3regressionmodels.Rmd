...
table(dat$bathrooms)
```
It isn't clear to me why this feature contains decimal numbers (e.g. 2.25). 
Nonetheless, the data is slightly left-skewed
...
Data distribution:
```{r}
summary(dat$condition)
table(dat$condition)
```
The distribution is right-skewed since the mean is larger than the median.
...
```
Most houses are in the 1001-2000 range.
...
Data distribution:
```{r}
summary(dat$sqft_living15_500_range)
```
